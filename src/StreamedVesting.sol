...
contract StreamedVesting is IStreamedVesting, OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(IERC20 _underlying, IERC20Burnable _vestedToken, IZeroLocker _locker, IBonusPool _bonusPool)
        external
        initializer
    {
    ...
    function _createVest(address from, address to, uint256 amount) internal whenNotPaused {
        vestedToken.burnFrom(from, amount);
        lastId++;

        vests[lastId] = VestInfo({who: to, id: lastId, amount: amount, claimed: 0, startAt: block.timestamp});
    ...
    function vestStatus(address who, uint256 index)
        external
        view
        returns (
            uint256 _id,
        ...
        )
    {
        ...
        uint256 pendingAmt = vest.amount - vest.claimed;
        _claimableWithPenalty = pendingAmt - ((pendingAmt * _penaltyAmt) / 1e18);
    }
    ...
    function _claimable(VestInfo memory vest) internal view returns (uint256) {
        ...
        return _claimable(vest.amount, vest.startAt, block.timestamp) - vest.claimed;
    }

    function _claimable(uint256 amount, uint256 startTime, uint256 nowTime) internal view returns (uint256) {
    ...
    function vestIds(address who) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < userVestCounts[who]; i++) {
