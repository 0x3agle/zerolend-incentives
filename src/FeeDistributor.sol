...
contract FeeDistributor is IFeeDistributor, ReentrancyGuard, Initializable {
    ...
    function initialize(address _votingEscrow, address _token) external initializer {
        WEEK = 7 * 86400;
    ...
    function checkpointToken() external override {
        require(((block.timestamp > lastTokenTime + TOKEN_CHECKPOINT_DEADLINE)), "cant checkpoint now");
    ...
    function _findTimestampUserEpoch(uint256 nftId, uint256 _timestamp, uint256 maxUserEpoch)
        internal
        view
        returns (uint256)
    {
    ...
    function _claim(uint256 nftId, uint256 _lastTokenTime) internal returns (uint256) {
        uint256 userEpoch = 0;
        ...
        if (weekCursor == 0) {
            userEpoch = _findTimestampUserEpoch(nftId, _startTime, maxUserEpoch);
        } else {
            userEpoch = userEpochOf[nftId];
        }

        if (userEpoch == 0) userEpoch = 1;

        IZeroLocker.Point memory userPoint = locker.userPointHistory(nftId, userEpoch);
        ...
        for (uint256 index = 0; index < 50; index++) {
            ...
            if (weekCursor >= userPoint.ts && userEpoch <= maxUserEpoch) {
                ...
                if (userEpoch > maxUserEpoch) {
                    ...
                    userPoint = locker.userPointHistory(nftId, userEpoch);
                }
            } else {
    ...
    function claim(uint256 id) external override nonReentrant returns (uint256) {
        return _claimWithChecks(id);
    }

    function claimMany(uint256[] memory nftIds) external override nonReentrant returns (bool) {
        if (block.timestamp >= timeCursor) {
            _checkpointTotalSupply(block.timestamp);
        }
